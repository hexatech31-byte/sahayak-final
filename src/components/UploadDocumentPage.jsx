...
export function UploadDocumentPage() {
  ...
  const handleAnalyze = async () => {
    ...
    const tenderTitle = selectedFile 
      ...
      : 'Supply and Installation of Technical Substation Equipment';

    try {
      const inputData = {
        fileName: selectedFile ? selectedFile.name : 'tender_specification_doc.pdf',
        ...
        description: `Auto-extracted requirement clauses from uploaded tender document: ${selectedFile?.name || 'tender_spec_doc.pdf'}`,
        quantity: '1,200 Sets'
      };
      ...
      if (addToast) {
        addToast('Document Registered', `Procurement #${newProc.id.slice(0, 8)} initialized in Firestore.`, 'info');
      }
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col antialiased selection:bg-orange-600 selection:text-white">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-col justify-start">
        ...
        <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-6 sm:p-10 text-left">
          ...
          {/* Card Header with Green Upload Icon */}
          <div className="flex items-center gap-4 mb-8">
            <div className="w-14 h-14 rounded-2xl bg-emerald-600 flex items-center justify-center text-white shadow-md shadow-emerald-500/20 shrink-0">
              <FileUp size={28} className="stroke-[2.2]" />
            </div>
            <div>
              ...
              <p className="text-sm text-slate-500 font-medium mt-0.5">
                Upload existing tender or specification document for analysis.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-8 sm:p-12 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-200 ${
                isDragging 
                  ...
                  : 'border-emerald-400/80 bg-white hover:bg-emerald-50/20 hover:border-emerald-500'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              <button
                ...
                className="px-6 py-2 rounded-xl border border-emerald-500 text-emerald-600 font-bold text-sm hover:bg-emerald-50 transition-colors shadow-xs cursor-pointer"
              >
                Choose File
              </button>
              ...
              {selectedFile && (
                <div 
                  ...
                  className="mt-4 px-4 py-2 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2 text-xs font-bold text-emerald-800"
                >
            ...
            {/* Information Panel */}
            <div className="bg-emerald-50/70 border border-emerald-100 rounded-2xl p-4 sm:p-4.5 flex items-start gap-3.5 text-slate-700 mt-6">
              <div className="w-5 h-5 rounded-full text-emerald-600 flex items-center justify-center shrink-0 mt-0.5">
                <Info size={20} />
              </div>
              <div className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                <p>You can upload any tender / specification document.</p>
                <p>Our AI model will analyze the document and extract key requirements, applicable standards and other relevant information.</p>
              </div>
            </div>
